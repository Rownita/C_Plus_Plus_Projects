...
bool deleteByVal(Node *&head, int x)
{
    ...
    else
    {
        ...
        //thisNode will be NULL if the insert value, we want to delete is not in the list
        if(thisNode == NULL)
        {
            return false;
        }
        //If your inserted value delete the first element
        if(thisNode == head)
        {
            head = thisNode->next;
        }
        //Since, you are deleting something from in between
        else
        {
...
int main()
{
    ...
    cout << "Enter the value to delete all its occurances:";
    cin >> x;
